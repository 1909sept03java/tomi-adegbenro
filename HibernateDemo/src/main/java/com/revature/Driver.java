...
public class Driver {
	...
	public static void main(String[] args) {
		...
		//Flashcard fd = new Flashcard("what else is Java?", "a programming language", new Topic(1, "coding"));
	...
	public static void setup() {
		...
		try (Session s = sf.openSession()) {
			...
			//s.get(StudySet.class, studySetId1).getFlashcards().add(new Flashcard(23, "", "", null)); //need to figure why not working
			tx.commit();
		}
	}
	
	public static void pop(SessionFactory sf) {
		//populate Flashcard and Topic DBs
		Session s1 = sf.openSession();
		Transaction tx = s1.beginTransaction();
		
		Flashcard fd = new Flashcard("what else is Java?", "a programming language", new Topic("coding"));
		s1.persist(fd);
