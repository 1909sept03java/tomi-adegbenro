...
R_DATE DATE;
BEGIN
    SELECT count(REQ_EMP) INTO E_EXIST FROM REQUEST WHERE REQUEST.REQ_EMP = E_ID;
    IF E_EXIST > 0 THEN
    R_DATE := SYSDATE;
    INSERT INTO BANKUSER (TITLE, SUMMARY, REQ_EMP, AMOUNT, REQ_DATE, STATUS) VALUES (TIT, SUMM, E_ID, AMT, SYSDATE);
        END IF;
    COMMIT;
    --EXCEPTION HANDLING
    EXCEPTION
