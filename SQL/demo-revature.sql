...
BEFORE INSERT ON BEAR --SPECIFY OPERATION, BEFORE/TABLE, AND TABLE
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON BEAR_TYPE --SPECIFY OPERATION, BEFORE/TABLE, AND TABLE
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON CAVE --SPECIFY OPERATION, BEFORE/TABLE, AND TABLE
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON BEEHIVE --SPECIFY OPERATION, BEFORE/TABLE, AND TABLE
FOR EACH ROW
BEGIN
...
--USER-DERFINES FUNCTIONS
--MAX OF TWO NUMERS (COULD WE USE THIS SYSTEM-DEFINED FUNCTION? YES WE COULD)
...
--q1
--creating a view
CREATE OR REPLACE VIEW VW_BEAR_WEIGHT_MAX(BEAR_NAME, WEIGHT)
AS
SELECT BEAR_NAME, WEIGHT FROM BEAR WHERE WEIGHT = (SELECT MAX(WEIGHT)FROM BEAR);

SELECT * FROM VW_BEAR_WEIGHT_MAX;


CREATE OR REPLACE FUNCTION SECOND_LARGEST
RETURN VARCHAR2
IS
BEAR_NAME VARCHAR2(100);
BEGIN
  SELECT BEAR_NAME FROM VW_BEAR_WEIGHT_MAX;
  --SELECT BEAR_NAME, WEIGHT FROM BEAR WHERE WEIGHT = (SELECT MAX(WEIGHT)FROM BEAR);
  RETURN BEAR_NAME;
END;
...
--Q2
--CREATIGN FUCNTION BEARS_LESS_THAN_AVERAGE
CREATE OR REPLACE FUNCTION BEARS_LESS_THAN_AVERAGE
RETURN SYS_REFCURSOR
IS
S SYS_REFCURSOR;
BEGIN
    OPEN S FOR
    SELECT BEAR_NAME FROM BEAR WHERE WEIGHT < (SELECT AVG(WEIGHT)FROM BEAR);
    RETURN S;
END;

--INVOKE BEARS_LESS_THAN_AVERAGE
DECLARE
S SYS_REFCURSOR;
SOME_ID BEAR.BEAR_ID%TYPE; -- DECLARES THAT SOME_ID IS OF THE SAME DATATYPE AS BEAR.BEAR_ID
SOME_NAME BEAR.BEAR_NAME%TYPE;
BEGIN
    S := BEARS_LESS_THAN_AVERAGE;
    LOOP
    FETCH S INTO SOME_ID, SOME_NAME; -- GRAB VALUES IN COLUMNS OF NEXT ROW IN ACTIVE SET
    EXIT WHEN S%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE('ID : '||SOME_ID||', NAME: '||SOME_NAME);
...
CREATE OR REPLACE  2ND_LARGEST
RETURN VARCHAR2(100)
IS
NAME VARCHAR2(100);
BEGIN
   --CREATE VIEW TABLE WITH NO MAX WEIGHT BEAR
   CREATE OR REPLACE VIEW NO_MAX_WEIGHT_BEAR(BEAR_NAME, WEIGHT)
   AS
   SELECT BEAR_NAME, WEIGHT
   FROM BEAR
   WHERE WEIGHT != (SELECT MAX(WEIGHT)FROM BEAR)
   --SELECT BEAR_NAME,WEIGHT FROM BEAR;
   SELECT BEAR_NAME, WEIGHT
   FROM NO_MAX_WEIGHT_BEAR
   WHERE WEIGHT = (SELECT MAX(WEIGHT)FROM NO_MAX_WEIGHT_BEAR);
END;
...
WEIGHT NUMBER;
BEGIN
    BEAR_NAME:= SECOND_LARGEST;
    DBMS_OUTPUT.PUT_LINE('SECOND LARGEST BEAR IS: '||NAME); 
END;
